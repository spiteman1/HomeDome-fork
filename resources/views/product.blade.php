...
<body>
         ...
         @if(isset($product['media']) && count($product['media']) > 1)
             <div class="thumbnail-gallery">
                 @foreach($product['media'] as $index => $media)
                     @if($media['type'] === 'image')
                         <img src="{{ asset($media['url']) }}" class="thumbnail {{ $index === 0 ? 'active' : '' }}" onclick="changeImage(this)">
                     @endif
                 @endforeach
             </div>
         @endif
     ...
     <div id="specifications" class="tab-content">
         ...
         <table class="specifications-table">
             <tbody>
                 ...
                 @endif
                 @if($product['energy_rating'])
                     <tr><th>Energy Rating</th><td>{{ $product['energy_rating'] }}</td></tr>
                 @endif

                 <tr><th>Availability</th><td>{{ $product['is_available'] ? 'Available' : 'Not Available' }}</td></tr>
                 <tr><th>Stock Quantity</th><td>{{ $product['stock_quantity'] }} units</td></tr>
             </tbody>
         </table>
     </div>
 ...
 <div id="viewerModal" class="viewer-modal">
     <div class="viewer-content">
         ...
         @if($modelUrl)
             <model-viewer src="{{ $modelUrl }}" alt="A 3D model of {{ $product['name'] }}" auto-rotate camera-controls ar shadow-intensity="1" style="width: 100%; height: 100%; background-color: #f5f5f5;"></model-viewer>
         @else
